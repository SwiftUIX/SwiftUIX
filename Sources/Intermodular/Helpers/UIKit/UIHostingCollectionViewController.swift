...
>: UIViewController, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    /*public func collectionView(
     ...
     var sizes = Array(cellContentSizeCache[originalIndexPath.section, default: [:]].values)
     let size = sizes.remove(at: originalIndexPath.row)
     sizes.insert(size, at: proposedIndexPath.row)
     
     for (index, size) in sizes.enumerated() {
     self.cellContentSizeCache[originalIndexPath.section, default: [:]][index] = size
     }
